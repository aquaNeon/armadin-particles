/* Blue Ridge / Armadin particle sequence — runtime
   Generated from particles-text-v5.html by tools/build-webflow.mjs.
...
(function () {
  ...
  // The camera.
  //
  // Zoom is a change to the cm -> px mapping, NOT a CSS transform on the
  // canvas. A transform would scale the type along with the field, and the
  // type is a spec: 140px at 1600 wide means 140px, at every scroll position.
  // Moving the mapping instead makes the world smaller in cm and the discs
  // bigger in px while the copy is still rasterised at exactly the size it
  // was authored at.
  //
  // The keep-out hole lives in display px and is baked from the same glyphs
  // that are drawn, so it does not move either. What has to follow the zoom
  // is the physics' idea of how many px a cm is worth -- TXT.zx/zy/zg below.
  // Get that wrong and the pack keeps its distance from where the type USED
  // to be, which is the one failure mode of this whole approach.
  var CAM = { z: 1, sx: 1, sy: 1 };     // sx/sy: device px per cm at zoom 1

  function applyCam() {
    var z = CAM.z;
    gl.uniform2f(U.uHalf, FRAME_W / (2 * z), FRAME_H / (2 * z));
    gl.uniform1f(U.uPxPerCm, Math.sqrt(CAM.sx * CAM.sy) * z);
    gl.uniform1f(U.uDpr, 1);
    // TXT is declared further down and is undefined on the first call, which
    // resize() makes before there is any copy to place.
    if (typeof TXT === "undefined" || !TXT) return;
    TXT.zx = TXT.sx * z; TXT.zy = TXT.sy * z; TXT.zg = TXT.sg * z;
    for (var i = 0; i < TXT.items.length; i++) boundsCm(TXT.items[i]);
  }
  ...
  function resize() {
    ...
    CAM.sx = canvas.width / FRAME_W;
    CAM.sy = canvas.height / FRAME_H;
    applyCam();
  }
  resize();
  ...
  var TXT = {
    ...
    items:    [],
    ready:    false,
    // sx/sy/sg: px per cm at zoom 1 -- what the TYPE is laid out and baked
    // against, so a px spec survives any camera move.
    // zx/zy/zg: px per cm as the camera currently has it -- what the PHYSICS
    // converts with, so the hole stays under the glyphs.
    sx: 1, sy: 1, sg: 1, zx: 1, zy: 1, zg: 1, W: 0, H: 0,
  };
  ...
  function parseCopy(list) {
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      out.push({
        ...
        // gmax is the high-water mark of ph, so the entrance scale only ever
  ...
  function bakeSdf(item) {
    ...
    item.live = gx1 >= gx0;
    // The box is stored in GRID cells, not cm. The silhouette is a fact about
    // the raster and never changes; how many cm it covers is a fact about the
    // camera and changes every frame the zoom is moving. boundsCm() converts,
    // and applyCam() reruns it.
    item.gx0 = gx0; item.gx1 = gx1; item.gy0 = gy0; item.gy1 = gy1;
    boundsCm(item);
    edt2d(fo, w, h);
    edt2d(fi, w, h);
  ...
  // Grid cells -> cm, at whatever the camera is doing now. Grown by the
  // clearance band plus the largest radius a disc can reach, so the bounding
  // box reject can never cut off a disc that is in range.
  function boundsCm(item) {
    if (!item.live) return;
    var R = TXT.raster, hw = TXT.W / 2, hh = TXT.H / 2;
    var zx = TXT.zx * R, zy = TXT.zy * R;
    var cx0 = (item.gx0 - hw) / zx, cx1 = (item.gx1 - hw) / zx;
    var cy0 = (hh - item.gy1) / zy, cy1 = (hh - item.gy0) / zy;
  ...
  //
  // `s` is the phrase's current scale. A shape scaled by s about its centre is
  // the same shape sampled at p/s with every distance multiplied by s -- the
  // same identity the exit beat already uses for translation, which is why
  // neither one rebakes. A uniform scale does not rotate normals, so the
  // gradient comes back unchanged.
  var probeD = 0, probeX = 0, probeY = 0;
  function probe(sdf, x, y, s) {
    var R = TXT.raster;
    if (!(s > 0.02)) s = 0.02;                 // the query divides by it
    // Anchored on the grid CENTRE rather than its corner. (x + FRAME_W/2) * sx
    // only lands on the centre while sx is exactly W/FRAME_W, which stops
    // being true the moment the camera zooms -- the whole field would slide
    // sideways off the copy.
    var gx = TXT.W / 2 + (x / s) * TXT.zx * R;
    var gy = TXT.H / 2 - (y / s) * TXT.zy * R; // cm +y is up, canvas y is down
    probeD = sdfAt(sdf, gx, gy) * s;
  ...
  function textForce(dt) {
    ...
    for (var t = 0; t < items.length; t++) {
      var it = items[t];
      if (!it.sdf || !it.live || (it.pw <= 0.002 && it.rel <= 0.002)) continue;
      var sdf = it.sdf, wgt = it.pw, rel = it.rel;
  ...
  function textProject() {
    ...
    var items = TXT.items, invSg = 1 / TXT.zg, sg = TXT.zg;
    for (var t = 0; t < items.length; t++) {
      var it = items[t];
      ...
      for (var i = 0; i < N; i++) {
        ...
        // but only once the shape has finished growing. During the entrance
        // the boundary is still moving outward through the pack, and ejecting
        // at full strength there is precisely the punch the growth exists to
  ...
  // The hole leads the copy.
  //
  // The physics weight and the opacity are two read-outs of one phase. The
  // hole opens and grows over the first `delay` of the phase with the type at
  // zero, and the type fades in over the rest at full size -- so the pack is
  // swept aside before there is anything to see, and what the reader watches
  // is copy appearing in a void that is already open. Scaling type in front of
  // the reader was the wrong picture; scaling a hole nobody can see is the
  // right one.
  //
  // On the way out the order reverses for free: opacity is gone by the time
  // the phase is back down to `delay`, and the hole releases over what is
  ...
  // Camera zoom. Three terms, added, all of them always on.
  //
  //   early  the cue. Small and front-loaded, because its only job is to
  //          answer "did that do anything" on the first notch of the wheel.
  //          It is spent inside the first eighth of the rail.
  //   drift  linear in p, the whole way down. This is what makes the move
  //          continuous: a zoom that settles at 0.30 and then holds for two
  //          thirds of the sequence reads as the page freezing again, which
  //          is the exact impression the early stage was added to avoid.
  //   late   the extra as the card comes over the top.
  //
  // early and drift are deliberately small. Everything from load to the end
  // of the copy is a reader following type, and the camera creeping under
  // that is competing with it -- all those beats need from the zoom is proof
  // that the scroll is connected to something. Between them they are worth
  // +2.1% across three quarters of the rail, against +6.75% in the last
  // quarter alone. The card arriving is the only place the move is meant to
  // be noticed as a move.
  //
  // Additive rather than sequenced, so none of them has to know about the
  // others and there is no seam where one hands over to the next. Every term
  // is non-decreasing in p, so the total is too.
  //
  // The zoom moves the CAMERA, not the picture. It is two uniforms -- see
  // applyCam -- so the world gets smaller in cm and the discs get bigger in
  // px, while the copy is still rasterised at exactly the px size it was
  // authored at and its keep-out hole is still cut from those same glyphs.
  //
  // Scaling the whole thing with a CSS transform would have been one line,
  // and it was the first version, but it scales the type too: a 140px
  // headline is a spec, not a suggestion, and it should read 140px at every
  // scroll position. Type also resamples badly -- a rasterised canvas pushed
  // through a compositor scale is soft in a way nothing else on the page is.
  //
  // The price is that the physics has to be told about the new cm-per-px, in
  // three places (probe, textForce, textProject, all via TXT.zx/zy/zg) plus
  // the bounding boxes. Get that wrong and the pack holds its distance from
  // where the type used to be. bench/text-v4-zoom.mjs is there for that.
  var ZOOM = {
    early: num(d.zoomEarly, 0.010),       // gain by the end of the early beat
    drift: num(d.zoomDrift, 0.015),       // gain across the whole rail
    late:  num(d.zoomLate,  0.060),       // extra gain over the late beat
    a: [num(d.beatZoom0, 0), num(d.beatZoom1, 0.12)],
    b: [num(d.beatZoomLate0, 0.76), num(d.beatZoomLate1, 1.00)],
    z: 1,
  };
  ...
  var RED = {
    on:    false,
    src:   null,      // the phrase the wave is anchored to; box re-read each frame
    r:     0,
  ...
  // Captures WHICH phrase the wave is anchored to, not the numbers. The
  // numbers are cm, and cm move under the camera -- re-reading the box each
  // frame is what keeps the near-copy boost on the copy at any zoom. The
  // phrase's `live` flag outlives its opacity, so the reference stays good
  // long after the headline itself is gone.
  ...
  function redFocus(items) {
    if (!RED.live || !RED.src) return;
    var src = RED.src, ss = src.scl;
    RED.cx = (src.cx0 + src.cx1) * 0.5 * ss;
    RED.cy = (src.cy0 + src.cy1) * 0.5 * ss;
    RED.hw = (src.cx1 - src.cx0) * 0.5 * ss;
    RED.hh = (src.cy1 - src.cy0) * 0.5 * ss;
  ...
  // Seed the pack with the hole already in it.
  //
  // The note this exists for: everyone likes how the field looks four or five
  // seconds in, once the shove has died down. That is not a note about the
  // arrival, it is a note about what the arrival leaves behind. A hole has to
  // displace its own area worth of discs; the pack is effectively
  // incompressible, so that area piles into a dense rim just outside the copy
  // and then travels outward as a wave. Measured at 2.9x the frame average
  // one second in, and still 1.3x at eight seconds -- it does not settle so
  // much as slowly leave.
  //
  // No amount of easing the arrival fixes that, because the displaced area is
  // conserved whatever route it takes. The only way to not have a wave is to
  // never make one: place the discs outside the copy in the first place.
  //
  // Rejection sampling against the same probe the physics uses, so "outside"
  // means exactly what it means everywhere else. Runs once, on the first
  // frame, after the timeline has decided which phrases are up -- so a reader
  // who refreshes halfway down the rail is seeded against the phrase that is
  ...
  function seedHoles() {
    if (!TXT.ready) return;
    var items = TXT.items, i, t, any = false;
    ...
    var moved = 0;
    for (i = 0; i < N; i++) {
      if (!seedBlocked(i, px[i], py[i])) continue;
      for (var k = 0; k < 48; k++) {
        var nx = (rand() - 0.5) * SCENE.emitterW;
        var ny = (rand() - 0.5) * SCENE.emitterH;
        if (seedBlocked(i, nx, ny)) continue;
        px[i] = nx; py[i] = ny; vx[i] = 0; vy[i] = 0;
        moved++;
        break;
      }
    }
    // Landing a fifth of the pack into a field that is already full leaves
    // local overlaps everywhere. They are local, though -- the same area went
    // back in that came out, so there is no gradient for a wave to run down --
    // and a few relaxation trips clear them before the first paint.
    var trips = Math.max(0, num(d.seedSettle, 8) | 0);
    for (var q = 0; q < trips; q++) { separate(1); textProject(); }
    SEED.moved = moved;
  }
  ...
  function scrollTimeline() {
    ...
    for (i = 0; i < n; i++) {
      ...
      // Physics reads the phase straight. Opacity reads the tail of it.
      var pv = itp.ph;
      itp.pw = pv * pv * (3 - 2 * pv);
      var ov = (pv - TWEEN.delay) / (1 - TWEEN.delay);
      ov = ov < 0 ? 0 : (ov > 1 ? 1 : ov);
      itp.w = ov * ov * (3 - 2 * ov);
      if (itp.ph > itp.gmax) itp.gmax = itp.ph;
      itp.scl = growOf(itp);
    }
    ...
    for (i = 0; i < n; i++) {
      ...
      if (itm.el) {
        ...
        // translate then scale, read right to left: the scale is about the
        // element centre and the travel is applied after it, which is the
        // same order the probe undoes -- (y - offY) / scl.
        var tr = itm.offY ? "translateY(" + (-itm.offY * TXT.zy).toFixed(1) + "px) " : "";
        if (itm.scl !== 1) tr += "scale(" + itm.scl.toFixed(4) + ")";
        itm.el.style.transform = tr;
      }
    }

    // Camera. One scale for the canvas and the text layer together, written
    // only when it actually changes so a parked reader is not handed a fresh
    // transform string sixty times a second.
    // Eased OUT, not smoothstepped -- so this does not go through ramp(),
    // which is a smoothstep. Smoothstep is flat at both ends, and the
    // flat end here is the one that matters: under it the first 5% of the
    // rail buys +0.4% of scale, which is nothing, and the reader still gets
    // no answer to "did that do anything". Ease-out spends the movement
    // early -- 5% of the rail is already a third of the travel -- and then
    // arrives gently, which is the right shape for a cue whose whole job is
    // to land on the first notch of the wheel.
    var e = (p - ZOOM.a[0]) / ((ZOOM.a[1] - ZOOM.a[0]) || 1e-6);
    e = e < 0 ? 0 : (e > 1 ? 1 : e);
    e = 1 - (1 - e) * (1 - e);
    var z = 1 + ZOOM.early * e
              + ZOOM.drift * p
              + ZOOM.late  * ramp(p, ZOOM.b[0], ZOOM.b[1]);
    if (z !== ZOOM.z) {
      ZOOM.z = z;
      CAM.z = z;
      applyCam();
    ...
    if (SCR.sticky) {
      ...
      SCR.sticky.style.setProperty("--brg-zoom", ZOOM.z.toFixed(4));
    }
  ...
  // phrases never get it -- they are busy opening their own hole.
    for (i = 0; i < n; i++) {
      ...
      itm.prevW = itm.w;
      if (itm.w > itm.gmax) itm.gmax = itm.w;
      itm.scl = growOf(itm);
      if (itm.el) {
        itm.el.style.opacity = itm.w;
        itm.el.style.transform = itm.scl === 1 ? "" : "scale(" + itm.scl.toFixed(4) + ")";
      }
    }
  }
  ...
  // The entrance.
  //
  // A phrase used to open its hole at full size and merely fade the FORCE in
  // over the tween. That is not soft: textProject ejects any disc actually
  // inside the silhouette outright, at full strength, as soon as w clears
  // 0.05 -- so within about two frames of load every disc standing where the
  // headline will be is teleported clear of it in one step. Ramping the force
  // cannot hide a positional correction.
  //
  // Growing the shape instead turns that single punch into a sweep: the
  // boundary passes each disc once, and each disc is only ever slightly
  // inside it, so the eject has almost nothing to do. Same final displacement,
  // spread over the tween instead of spent in a frame.
  ...
  function textLayout() {
    ...
    TXT.zx = TXT.sx * CAM.z; TXT.zy = TXT.sy * CAM.z; TXT.zg = TXT.sg * CAM.z;
  ...
  function tick(now) {
    ...
    if (!SEED.done) { SEED.done = true; seedHoles(); }
  ...
  window.BRG = {
    ...
    intro: function () {
      for (var i = 0; i < TXT.items.length; i++) {
        var it = TXT.items[i];
        it.ph = it.tgt;
        it.pw = it.ph * it.ph * (3 - 2 * it.ph);
        it.w = it.pw;
        if (it.ph > it.gmax) it.gmax = it.ph;
        it.scl = growOf(it);
      }
    },
    state: function () {
